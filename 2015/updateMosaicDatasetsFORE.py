...
def updateMosaic(inputFolder, workspaceName, mosaicName):
    ...
    # Remove all raster files before udpating it
    logging.debug("Deleting rasters from Mosaic Dataset..." + mosaicName)
    arcpy.RemoveRastersFromMosaicDataset_management(in_mosaic_dataset=mosaicPath,
    ...
    with arcpy.da.UpdateCursor(mosaicPath, fields) as cursor:
        for row in cursor:
            ...
            parts =  rasterFileName.split("_")  # Ex: IT_Meteo_Forecast_TMax_2015_246_plus1.tif
            row[1] = parts[3].upper()	# TMax
            row[2] = parts[4]	        # 2015
            day = int(parts[5])	        # 246
            if len(parts) == 6:
                dateValue = datetime.datetime(int(parts[4]), 1, 1) + datetime.timedelta(day-1)
            else:
                plus = parts[6]
                dayplus = int(plus[-1:]) # extract 1 from 'plus1'
                dateValue = datetime.datetime(int(parts[4]), 1, 1) + datetime.timedelta(day+dayplus-1)          
