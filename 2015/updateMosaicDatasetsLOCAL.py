...
# Example:  python UpdateMosaicDatasetsLOCAL.py c:/ERMES/PRODUCTS/IT_2015 foldersIT_2015_LOCAL.txt IT_2015_LOCAL.log
...
def updateMosaic(inputFolder, workspaceName, mosaicName):
    ...
    with arcpy.da.UpdateCursor(mosaicPath, fields) as cursor:
        for row in cursor:
    		...
    		parts =  rasterFileName.split("_")  # Ex: IT_LAI_ETM_2015_099.tif
    		row[1] = parts[2].upper()	# etm or oli
    		row[2] = parts[3]	        # 2015
    		day = int(parts[4])	        # 001
    		dateValue = datetime.datetime(int(parts[3]), 1, 1) + datetime.timedelta(day-1)
